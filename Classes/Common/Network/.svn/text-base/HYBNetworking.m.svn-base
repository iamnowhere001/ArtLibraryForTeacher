...
static HYBRequestType  sg_requestType  = kHYBRequestTypePlainText;//kHYBRequestTypeJSON;修改默认requestType为plainText，以解决很多小伙伴们出现后台接收不到参数的问题！
...
+ (HYBURLSessionTask *)_requestWithUrl:(NSString *)url
  ...
  if (httpMethod == 1) {
      if (sg_cacheGet) {
          ...
          if (refreshCache) {// 获取缓存 ^&$%$$ no ！refreshCache
              id response = [HYBNetworking cahceResponseWithURL:absolute
                                                     parameters:params];
              if (response) {
                  if (success) {
...
//  }
  ...
  HYBURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
      ...
      /* 如果图片大小大于4M 才进行压缩 否则传原图  ---->头像2M  扩展*/
      
      NSData *imageData = nil;//UIImageJPEGRepresentation(image, 0.7); // 1.0 ------> jpeg 原图大小
      if ([name isEqualToString:@"Library"]) {
          // 图片 大小小于4M
          imageData = UIImageJPEGRepresentation(image, 0.6); // 优化
      } else {
         // 头像 大小小于2M
          imageData = UIImageJPEGRepresentation(image, 0.4);
      }
      HYBAppLog(@"----------kb------ %luKB",imageData.length/1024);
...
+ (AFHTTPSessionManager *)manager {
  ...
  switch (sg_requestType) {
    ...
    case kHYBRequestTypePlainText: {// 这个应该是默认请求类型
