...
@interface DXPopover ()<UITableViewDataSource, UITableViewDelegate>
...
- (void)initTableView{
    ...
    blueView.frame = CGRectMake(0, 0, 110, 150);//
...
- (void)showAtPoint:(CGPoint)point{
    ...
    [self showAtPoint:point popoverPostion:DXPopoverPositionDown withContentView:self.tableView inView:[UIApplication sharedApplication].keyWindow];
}
...
- (void)showAtPoint:(CGPoint)point popoverPostion:(DXPopoverPosition)position withContentView:(UIView *)contentView inView:(UIView *)containerView
{
    ...
    switch (self.maskType) {
        case DXPopoverMaskTypeBlack:
            maskColor = [UIColor colorWithWhite:0.0 alpha:0.1];
            break;
        case DXPopoverMaskTypeNone:
            maskColor = [UIColor clearColor];
            break;
        case DXPopoverMaskTypeWhite:
            maskColor = [UIColor colorWithWhite:1.0 alpha:0.55];
            break;
        default:
            break;
    }
    ...
    [self.blackOverlay addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchUpInside];//dismiss
...
- (void)dismiss{
    ...
    if (self.superview) {
        [UIView animateWithDuration:self.animationOut delay:0.0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            // 通知
            // 1.注册通知 2.监听 3.移除通知 比较麻烦
            //[[NSNotificationCenter defaultCenter] postNotificationName:@"PopoverViewDissmiss" object:nil];
     ...
     */
    // userdefault
    [[NSUserDefaults standardUserDefaults]setBool:NO forKey:@"kPopViewDismiss"];
    [[NSUserDefaults standardUserDefaults]synchronize];
    
    //BOOL isDismiss = [[NSUserDefaults standardUserDefaults]boolForKey:@"kPopViewDismiss"];
    //NSLog(@"---333---%d",isDismiss);
