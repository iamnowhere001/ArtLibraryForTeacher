...
@implementation ChooseSemesterCustomView
...
- (void)createUI {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        weakSelf.scrollView.contentSize = CGSizeMake(weakSelf.scrollView.bounds.size.width, weakSelf.semesterArr.count * 55);
        for (NSInteger i = 0; i < weakSelf.semesterArr.count; i++) {
            ...
            UIView *buttonLine = [[UIView alloc]initWithFrame:CGRectMake(0, button.frame.size.height - 0.5, button.frame.size.width, 0.5)];
            UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(button.frame.size.width - 50, 14, 27, 27)];
            
            dispatch_async(dispatch_get_main_queue(), ^{
               
                [button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
                SemesterModel *model = weakSelf.semesterArr[i];
                [button setTitle:model.semesterName forState:UIControlStateNormal];
...
- (void)choose:(UIButton *)button{
    ...
    SemesterModel *model = self.semesterArr[button.tag - 10];
    NSString *idStr = [NSString stringWithFormat:@"%ld",button.tag-9];
    if ([idStr integerValue] != self.select) {
        
        if (self.chooseBlock) {
            self.chooseBlock(model.semesterId,idStr);
        }
